...
export class MemoryBotEngine {
  ...
  handleMessage(bot, sender, channel, isDirect, msg, done) {
    ...
    const parseAndReply = (key, value, tell = null) => {
    ...
    const update = (key, value) => {
      ...
      this.store.setFactoid(team, key, value, lastEdit, err => {
        if (err) {
          log.error(err);
    ...
    // Status
    if (isDirect && (/^status\??/ig).test(msg)) {
      const bool = x => x ? ':ballot_box_with_check:' : ':white_medium_square:';
      this.store.countFactoids(team, (err, count) => {
        if (err) log.error(err);
        reply(`\
*Status*
...
`
    ...
    // Help
      ...
      reply(`Hi alice, I'm a MemoryBot. I remember things and then recall them later when asked. Check out my home page for more information: https://statico.github.io/memorybot/`);
      done();
      return;
    ...
    // Settings
      ...
      series([
        cb => this.store.setMetaData(team, key, value, cb),
        cb => this.store.updateBotMetadata(bot, cb)
      ], err => {
        if (err) {
          log.error(err);
    ...
    // A greeting?
      ...
      if (shouldReply || isVerbose) {
        reply(this.oneOf(GREETINGS).replace(/\$who/ig, sender));
      }
      done();
      return;
    ...
    // Getting literal factoids
      ...
      this.store.getFactoid(team, key, (err, current) => {
        if (current != null) {
          reply(`${key} ${current}`);
        } else {
    ...
    // Getting regular factoids
      ...
      if (IGNORED_FACTOIDS.includes(key.toLowerCase())) return done();
      this.store.getFactoid(team, key, (err, current) => {
        if (current != null) {
    ...
    // Deleting factoids
      ...
      this.store.deleteFactoid(team, key, err => {
        if (err) {
          log.error(err);
    ...
    // Tell users about things
      ...
      bot.api.users.list({}, (err, res) => {
        ...
        this.store.getFactoid(team, key, (err, value) => {
          if (value == null) {
            reply(this.oneOf(I_DONT_KNOW));
            done();
            return;
    ...
    // Karma query
      ...
      this.store.getKarma(team, key, (err, current) => {
    ...
    // Karma increment/decrement
      ...
      this.store.getKarma(team, key, (err, current) => {
        ...
        this.store.setKarma(team, key, value, err => {
          if (err) {
            log.error(err);
    ...
    // Getting regular factoids, last chance
      ...
      if (IGNORED_FACTOIDS.includes(msg.toLowerCase())) return done();
      this.store.getFactoid(team, msg, (err, value) => {
        if (value != null) {
